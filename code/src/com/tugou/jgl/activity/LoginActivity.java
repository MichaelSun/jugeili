...
//		    	}
    ...
    private void Login(final String email, final String pwd) {
        CustomThreadPool.getInstance().excute(new TaskWrapper(new Runnable() {
            ...
            public void run() {
                try {
                    final LoginResponse response = InternetUtils.request(getApplicationContext(), new LoginRequest(pwd, email));//versionCode));
                    if (response != null) {
                        if (Integer.valueOf(response.status) == 1) {
                            Debug.LOGD("login success!!");

                            SettingManager.getInstance().setBooleanProperty("login", true);
                            SettingManager.getInstance().setIntProperty("user_id", response.user_id);
